...
export const h5Login = function(type = "judge", callback) {
	...
	if (getBrowser() == "微信") {
	// uni.showToast({
	// 			title: "欢迎回来22222"+JSON.stringify(getRequest),
	// 			icon: "none"
	// 		});
		if (getRequest.code) {
			...
			if(isGetOpenId){
				...
				$http.get('/wechatLogin', httpData).then(res => {
					console.log(res,"/wechatLogin/wechatLogin/wechatLogin/wechatLogin/wechatLogin")
					console.log(store)
					// res.token = res.datas;
					// res.datas = '';
