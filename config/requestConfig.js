...
$http.requestStart = function (options) {
  ...
  if (storeUserInfo.token) {
    options.header['Qxsztoken'] = storeUserInfo.token
  }
  return options
}
//请求结束
...
$http.dataFactory = async function (res) {
  ...
  if (res.response.statusCode && res.response.statusCode == 200) {
    ...
    if (httpData.code === '200') {
      ...
      let buttons = ''
      if (httpData.buttons) {
        buttons = httpData.buttons
      }
      uni.setStorageSync('buttons', JSON.stringify(buttons))
      return Promise.resolve(httpData.datas)
    } else if(httpData.code === '207'){
      // 返回到登录页
      uni.navigateTo({
